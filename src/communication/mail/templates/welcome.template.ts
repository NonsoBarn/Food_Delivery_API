 ...
 *
 * KEY LEARNING: Templates as plain TypeScript functions
 * ======================================================
 * Templates are just functions that accept data and return an HTML string.
 * No template engine (Handlebars, EJS, Pug) needed at this stage.
 *
 * Advantages:
 * - Full TypeScript type safety on the data
 * - Easy to test (just call the function with mock data)
 * - Zero extra dependencies
 *
 * When to graduate to a template engine:
 * - You need template inheritance (base layout + child templates)
 * - Designers want to edit templates without touching TypeScript
 * - You need i18n (translations) inside templates
 *
 * The `role` parameter lets us customise the copy for different user types.
 */
export function welcomeEmailHtml(email: string, role: string): string {
  const roleMessages: Record<string, string> = {
    CUSTOMER: 'Browse local restaurants and get food delivered to your door.',
    VENDOR:
      'Set up your store, add your menu, and start receiving orders today.',
    RIDER:
      'Accept delivery requests, track your earnings, and deliver with ease.',
